...
<body class="from-10% via-30% to-90% mx-auto mt-10 w-full text-slate-700">
    ...
    @if (session('success'))
    <div role="alert" class="my-8 rounded-md border-l-4 border-green-300 bg-green-100 p-4 text-green-700 opacity-75 w-3/4">
        <p class="font-bold">Success!</p>
        <p>{{ session('success') }}</p>
    </div>
    @endif
    @if (session('error'))
    <div role="alert" class="my-8 rounded-md border-l-4 border-red-300 bg-red-100 p-4 text-red-700 opacity-75 w-3/4">
        <p class="font-bold">Error!</p>
        <p>{{ session('error') }}</p>
    </div>
    @endif

    <div {{ $attributes->class(['w-3/4 mx-auto h-full']) }}>
        {{ $slot }}
    </div>
</body>
